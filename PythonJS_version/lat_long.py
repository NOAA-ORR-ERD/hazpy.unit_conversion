...
def modf(x):
    ...
    re-implimentation of math.modf, for PythonJS
    """
    if x == 0.0 and repr(x)[0] == '-': # it's -0.0 -- weird case
        return (-0.0, -0.0)
    i = round(x)
    return (x-i, i)
...
def signbit(value):
    """
    uses the repr kludge to check for the sign.

    this captures -0.0, for instance

    This version for compatibility with PythonJS
    """
    return repr(value)[0] == "-"
...
class Latitude:
    ...
    def __repr__(self):
        try:
            return u"%s(%f)" % (self.__class__.__name__, self.value)
    ...
    def format(self, style):
        ...
        returns formatted value as Unicode string with the degree symbol embedded
...
#DEGREES = u"\xb0"     # "DEGREE SIGN"
#MINUTES = u"\u2032"   # "PRIME"
#SECONDS = u"\u2033"   # "DOUBLE PRIME"


LAT_POSITIVE_DIRECTION = u"North"
LAT_NEGATIVE_DIRECTION = u"South"
LON_POSITIVE_DIRECTION = u"East"
LON_NEGATIVE_DIRECTION = u"West"

FORMAT1 = u"{:.2f}\N{DEGREE SIGN} {}"
FORMAT2 = u"{:.0f}\N{DEGREE SIGN} {:.2f}\N{PRIME} {}"
FORMAT3 = u"{:.0f}\N{DEGREE SIGN} {:.0f}\N{PRIME} {:.2f}\N{DOUBLE PRIME} {}"
